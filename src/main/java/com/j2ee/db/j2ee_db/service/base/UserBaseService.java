...
import com.j2ee.db.j2ee_db.service.RolesService;
...
public class UserBaseService {
	...
	public User insert(User obj) {
		...
		String sql = "INSERT INTO `user` (`_id`, `username`, `password`, `mail`,`name`,`phone` )	VALUES (:id, :username , :password, :mail,:name,:phone )";
    ...
    //CRUD - FIND BY Username
    	
	public List<User> findByusername(Long idusername) {
		
		String sql = "select * from `User` WHERE `username` = :idusername";
		
	    SqlParameterSource parameters = new MapSqlParameterSource()
		.addValue("idusername", idusername);
	...
	public User update(User obj, Long id) {
		...
		String sql = "UPDATE `User` SET `mail`,`name`,`phone` WHERE `_id`=:id";
		SqlParameterSource parameters = new MapSqlParameterSource()
		    .addValue("id", obj.get_id())
			.addValue("mail", obj.getMail())
			.addValue("name", obj.getName())
