...
//CRUD METHODS
    ...
    @Secured({ "ROLE_PRIVATE_USER" })
	@RequestMapping(value = "/user/findByusername/{key}", method = RequestMethod.GET, headers = "Accept=application/json")
	public List<User> findByusername(@PathVariable("key") Long idusername) {
		List<User> list = userService.findByusername(idusername);
		return list;
	}
